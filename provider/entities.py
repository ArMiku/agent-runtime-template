...
if TYPE_CHECKING:
    # SDK response types are referenced only in annotations; guarding them avoids a
    # hard runtime dependency on google-genai (Gemini is deferred, see design.md §11)
    # while anthropic/openai remain runtime deps via the provider sources.
...
from agent_runtime.media.media_utils import MediaResolver

# NOTE: A concrete conversation/persistence model is intentionally not bundled —
# persistence is an application concern, exposed via the injectable ``ContextStore``
# seam. The field stays as ``Any`` so callers can thread their own conversation object
# through ``ProviderRequest``.
...
class ProviderRequest:
    ...
    """关联的对话对象（不透明；持久化经 ``ContextStore`` 接缝，见 design.md §6）"""
