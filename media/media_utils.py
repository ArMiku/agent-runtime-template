...
from agent_runtime.foundation.io_utils import download_file
from agent_runtime.foundation.paths import get_temp_dir


class AudioCodecHook(Protocol):
    """Optional audio codec hook for QQ/WeChat ``silk`` voice encode/decode.

    The silk codec (``silk-python`` / ``tencent_record_helper``) is QQ/WeChat-specific
    and is **not** bundled — it would violate framework independence. Consumers that need
    QQ silk voice register an implementation via :func:`set_audio_codec_hook`.
    """

    async def encode_silk(self, wav_path: str, silk_path: str) -> int:
        """Encode a WAV file to Tencent silk; return the duration in ms."""
        ...

    async def decode_silk(self, silk_path: str, wav_path: str) -> str:
        """Decode a Tencent silk file to WAV; return the WAV path."""
        ...


_audio_codec_hook: AudioCodecHook | None = None


def set_audio_codec_hook(hook: AudioCodecHook | None) -> None:
    """Register (or clear) the optional silk audio codec hook."""
    global _audio_codec_hook
    _audio_codec_hook = hook


def get_audio_codec_hook() -> AudioCodecHook | None:
    """Return the registered silk codec hook, or ``None`` if none is registered."""
    return _audio_codec_hook
...
def file_uri_to_path(file_uri: MediaRefStr) -> str:
    ...
    if len(path) >= 4 and path[0] == "/" and path[2] == ":" and path[1].isalpha():
        ...
        # Some sources generate file:////path for POSIX absolute paths.
        path = "/" + path.lstrip("/")
...
def _temp_media_path(media_type: str, suffix: str) -> Path:
    """Create a unique path under the runtime temp directory for materialized media."""
            ...
            if self.media_type == "audio":
                ...
                if audio_format == "tencent_silk":
                    ...
                    try:
                        ...
                        hook = get_audio_codec_hook()
                        if hook is None:
                            raise ValueError(
                                "tencent_silk output requested but no AudioCodecHook "
                                "is registered (silk/QQ-voice codec is optional; see "
                                "agent_runtime.media.set_audio_codec_hook)"
                            )
                        duration = await hook.encode_silk(str(wav_path), str(silk_path))
                        if duration <= 0:
...
async def convert_audio_to_opus(audio_path: str, output_path: str | None = None) -> str:
    ...
    Args:
        ...
        output_path: Optional output file path. When omitted, a temporary path is
            created under the runtime temp directory.
...
async def ensure_wav(audio_path: str, output_path: str | None = None) -> str:
    ...
    Args:
        ...
        output_path: Optional destination path. When omitted, conversion helpers
            create a temporary file under the runtime temp directory.
    ...
    if audio_type == "silk":
        hook = get_audio_codec_hook()
        if hook is None:
            logger.warning("silk audio detected but no AudioCodecHook registered; returning path unchanged")
            return audio_path
        if output_path is None:
            temp_dir = get_temp_dir()
            os.makedirs(temp_dir, exist_ok=True)
            output_path = os.path.join(temp_dir, f"media_audio_{uuid.uuid4().hex}.wav")
        return await hook.decode_silk(audio_path, output_path)
...
async def ensure_jpeg(image_path: str, output_path: str | None = None) -> str:
    ...
    Args:
        ...
        output_path: Optional destination path. When omitted, a temporary file under
            The runtime temp directory is created for converted JPEG output.
    ...
    def convert_image_to_jpeg() -> str:
        ...
        with PILImage.open(image_path) as opened_img:
            try:
                ...
                if opened_img.mode in {"RGBA", "LA"} or (opened_img.mode == "P" and "transparency" in opened_img.info):
...
async def extract_video_cover(
    ...
    Args:
        ...
        output_path: Optional output image path. When omitted, a temporary JPEG
            path is created under the runtime temp directory.
