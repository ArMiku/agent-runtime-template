...
from agent_runtime.message.components import (
    At,
    AtAll,
...
# NOTE: Only the neutral :class:`MessageChain` container is retained as the agent output
# carrier (design.md §4 decision 2 / spec "保留 MessageChain 输出载体"). Event-pipeline
# glue (CONTINUE/STOP result types, command-result aliases, plugin-handler return-type
# coupling) is intentionally out of scope — the runtime has zero references to it once
# ``FunctionTool.handler`` is retyped (design.md §5).
