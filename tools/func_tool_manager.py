...
MCP_INIT_TIMEOUT_ENV = "AGENT_RUNTIME_MCP_INIT_TIMEOUT"
ENABLE_MCP_TIMEOUT_ENV = "AGENT_RUNTIME_MCP_ENABLE_TIMEOUT"
...
class FunctionToolManager:
    def __init__(self) -> None:
        ...
        """All registered tools (plugin-defined + MCP tools). The template ships no builtin tools."""
    ...
    def get_func(self, name) -> FunctionTool | None:
    ...
    def get_full_tool_set(self) -> ToolSet:
        ...
        使用 ToolSet.add_tool 进行填充；同名工具的去重（active 优先、后加载覆盖）
        由 ``ToolSet.add_tool`` 统一处理。模板不含权限守卫与内置工具——使用方按需在
        自己的 ``BaseFunctionToolExecutor`` 或工具 wrapper 上实现 allow/deny/ask 三态
        （参考 design.md §5.3 / README 已知限制）。
        """
        tool_set = ToolSet()
    ...
    async def init_mcp_clients(self, raise_on_all_failed: bool = False) -> MCPInitSummary:
        ...
        Timeout behavior:
        - 初始化超时使用环境变量 AGENT_RUNTIME_MCP_INIT_TIMEOUT 或默认值。
        - 动态启用超时使用 AGENT_RUNTIME_MCP_ENABLE_TIMEOUT（独立于初始化超时）。
        """
    ...
    async def enable_mcp_server(
        ...
        Args:
            ...
            timeout: Timeout in seconds for initialization.
                Uses AGENT_RUNTIME_MCP_ENABLE_TIMEOUT by default (separate from init timeout).
