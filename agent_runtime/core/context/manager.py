...
class ContextManager:
    ...
    def __init__(
        ...
        if config.custom_compressor:
            ...
            self.compressor = TruncateByTurnsCompressor(truncate_turns=config.truncate_turns)
    ...
    async def process(self, messages: list[Message], trusted_token_usage: int = 0) -> list[Message]:
        ...
        try:
            ...
            if self.config.max_context_tokens > 0:
                ...
                if self.compressor.should_compress(result, total_tokens, self.config.max_context_tokens):
    ...
    async def _run_compression(self, messages: list[Message], prev_tokens: int) -> list[Message]:
        """
        ...
        # last check
        if self.compressor.should_compress(messages, tokens_after_summary, self.config.max_context_tokens):
            logger.info("Context still exceeds max tokens after compression, applying halving truncation...")
