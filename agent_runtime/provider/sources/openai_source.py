...
import agent_runtime.message.components as Comp
from agent_runtime import logger
from agent_runtime.core.message import (
    AudioURLPart,
    ContentPart,
    ImageURLPart,
    Message,
...
# (AbstractProvider.media_resolver, default DefaultMediaResolver); no direct
# media_utils import keeps the runtime free of any hard media-utils dependency.
from agent_runtime.foundation.network import (
    create_proxy_client,
    is_connection_error,
    log_connection_failure,
)
from agent_runtime.foundation.string_utils import normalize_and_dedupe_strings
from agent_runtime.message.message_event_result import MessageChain
from agent_runtime.provider.entities import LLMResponse, TokenUsage, ToolCallsResult
...
class ProviderOpenAIOfficial(Provider):
    ...
    async def _image_ref_to_data_url(
        ...
        *,
        ...
        image_data = await self.media_resolver.resolve(
            image_ref,
            media_type="image",
            strict=mode == "strict",
        )
    ...
    async def _resolve_audio_part(self, audio_ref: str) -> dict | None:
        try:
            audio_data = await self.media_resolver.resolve(
                audio_ref,
                media_type="audio",
                strict=True,
            )
    ...
    async def _parse_openai_completion(self, completion: ChatCompletion, tools: ToolSet | None) -> LLMResponse:
        ...
        if choice.finish_reason == "content_filter":
            raise Exception(
                "API 返回的 completion 由于内容安全过滤被拒绝（由上游 API 拒绝，非本运行时）。",
            )
    ...
    async def encode_image_bs64(self, image_url: str) -> str:
        ...
        if image_data is None:
            raise RuntimeError(f"Failed to encode image data: {self.media_resolver.describe(image_url)}")
        return image_data
