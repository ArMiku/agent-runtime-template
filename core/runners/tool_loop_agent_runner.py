...
from agent_runtime.core.message import ImageURLPart, TextPart, ThinkPart
from agent_runtime.core.tool import FunctionTool, ToolSet
from agent_runtime.core.tool_image_cache import tool_image_cache
from agent_runtime.foundation.exceptions import EmptyModelOutputError
from agent_runtime.message.components import Json
from agent_runtime.message.message_event_result import (
    MessageChain,
)
from agent_runtime.provider.entities import (
    LLMResponse,
    ProviderRequest,
    ToolCallsResult,
)
from agent_runtime.provider.modalities import (
...
class _HandleFunctionToolsResult:
    ...
    @classmethod
    def from_tool_call_result_blocks(cls, blocks: list[ToolCallMessageSegment]) -> "_HandleFunctionToolsResult":
...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    ...
    async def reset(
        ...
        **kwargs: T.Any,
        ...
        self.request_context_manager = ContextManager(self.request_context_manager_config)
        ...
        if request.prompt is not None or request.image_urls or request.audio_urls or request.extra_user_content_parts:
    ...
    async def _assemble_request_context_for_provider(
        ...
        if (
            not modalities
        ):  # Unconfigured (None or empty list) defaults to support all modalities for backward compatibility
    ...
    async def _write_tool_result_overflow_file(
        ...
        *,
        ...
        safe_tool_call_id = (
            "".join(ch if ch.isalnum() or ch in {"-", "_", "."} else "_" for ch in tool_call_id).strip("._")
            or "tool_call"
        )
    ...
    async def _iter_llm_responses(self, *, include_model: bool = True) -> T.AsyncGenerator[LLMResponse, None]:
                    ...
                    with attempt:
                        try:
                            async for resp in self._iter_llm_responses(include_model=idx == 0):
                                ...
                                if resp.role == "err" and not has_stream_output and (not is_last_candidate):
    ...
    def _consume_follow_up_notice(self) -> str:
        ...
        for ticket in follow_ups:
            ...
            ticket.resolved.set()
        follow_up_lines = "\n".join(f"{idx}. {ticket.text}" for idx, ticket in enumerate(follow_ups, start=1))
    ...
    async def step(self):
        ...
        if llm_resp.role == "err":
            ...
            self._resolve_unconsumed_follow_ups()
            error_text = f"LLM 响应错误: {llm_resp.completion_text or '未知错误'}"
            yield AgentResponse(
                type="err",
                data=AgentResponseData(
        ...
        if llm_resp.tools_call_name:
            if self.tool_schema_mode == "skills_like":
                ...
                if not requery_resp.tools_call_name:
                    ...
                    logger.warning("skills_like tool re-query returned no tool calls; fallback to assistant response.")
                    if llm_resp.reasoning_content:
                        yield AgentResponse(
                            type="llm_result",
                            data=AgentResponseData(
            ...
            if cached_images:
                ...
                if supports_image:
                    ...
                    for cached_img in cached_images:
                        img_data = tool_image_cache.get_image_base64_by_path(cached_img.file_path, cached_img.mime_type)
                        if img_data:
                    ...
                    if image_parts:
                        ...
                        logger.debug(f"Appended {len(cached_images)} cached image(s) to context for LLM review")
    ...
    async def _handle_function_tools(
        ...
        for func_tool_name, func_tool_args, func_tool_id in zip(
            ...
            try:
                ...
                # 参数过滤判别器重锚（design.md §5.1）：按"是否声明了 schema"收口，
                # 而非依赖 handler 的存在性。声明了 properties 的工具（含
                # handler 工具与 MCP 工具）都按声明参数过滤；未声明 schema 的透传全部参数。
                    ...
                    if isinstance(resp, CallToolResult):
                        ...
                        for index, content_item in enumerate(res.content):
                            if isinstance(content_item, TextContent):
                                ...
                                yield _HandleFunctionToolsResult.from_cached_image(cached_img)
                                ...
                                if isinstance(resource, TextResourceContents):
                                    ...
                                    result_parts.append("The tool has returned a data type that is not supported.")
                        if result_parts:
                            inline_result = "\n\n".join(result_parts)
                            ...
                            _append_tool_call_result(
                                func_tool_id,
                                inline_result
                                + self._build_repeated_tool_call_guidance(func_tool_name, tool_call_streak),
                            )
                        ...
                        logger.warning(f"{func_tool_name} 没有返回值，或者已将结果直接发送给用户。")
                        self._transition_state(AgentState.DONE)
                        self.stats.end_time = time.time()
                        _append_tool_call_result(
                            func_tool_id,
                ...
                _append_tool_call_result(
                    func_tool_id,
                    f"error: {e!s}" + self._build_repeated_tool_call_guidance(func_tool_name, tool_call_streak),
                )
        ...
        if tool_call_result_blocks:
            yield _HandleFunctionToolsResult.from_tool_call_result_blocks(tool_call_result_blocks)
    ...
    def _build_tool_requery_context(
        ...
        instruction = self.SKILLS_LIKE_REQUERY_INSTRUCTION_TEMPLATE.format(tool_names=", ".join(tool_names))
    ...
    async def _resolve_tool_exec(
        ...
        if isinstance(self._tool_schema_param_set, ToolSet):
            param_subset = self._build_tool_subset(self._tool_schema_param_set, tool_names)
            if param_subset.tools and tool_names:
                ...
                if not llm_resp.tools_call_name and not self._has_meaningful_assistant_reply(llm_resp):
                    logger.warning(
    ...
    async def _iter_tool_executor_results(
        ...
        while True:
            if self._is_stop_requested():
                ...
                raise _ToolExecutionInterrupted("Tool execution interrupted before reading the next tool result.")
            ...
            try:
                ...
                if abort_task in done:
                    ...
                    raise _ToolExecutionInterrupted("Tool execution interrupted by a stop request.")
