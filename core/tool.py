...
class FunctionTool(ToolSchema, Generic[TContext]):
    ...
    handler: (
        Callable[
            ...,
            Awaitable[ToolExecResult] | AsyncGenerator[ToolExecResult, None],
        ]
        | None
    ) = None
    """A callable that implements the tool's functionality — the function-style tool
    definition path (design.md §5).

    Clean, event-free contract: the first positional argument is the
    :class:`~agent_runtime.core.run_context.ContextWrapper` carrying the caller's own
    ``TContext``; the callable returns a :data:`ToolExecResult`
    (``str | mcp.types.CallToolResult``), either as an awaitable or an async generator.

    The ergonomic "define a tool by passing a function" path uses a clean signature:
    the callable's first argument is a ``ContextWrapper[TContext]`` and it returns a
    :data:`ToolExecResult` — no ``event`` / ``MessageEventResult`` coupling.

    Note: this dataclass carries no host-application-specific fields (no
    ``handler_module_path`` / ``active`` / ``is_background_task``). Any ``active``-based
    filtering elsewhere degrades gracefully to "all tools active" via
    ``getattr(.., "active", True)``.
    """
